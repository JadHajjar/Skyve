...
internal class D_NotificationCenter : IDashboardItem
{
	...
	{
		DrawSection(e, applyDrawing, e.ClipRectangle.ClipTo(sectionHeight), Locale.Notifications, "Notification", out _, ref preferredHeight);
	...
	private void Draw(PaintEventArgs e, bool applyDrawing, ref int preferredHeight, INotificationInfo notification)
	{
		...
		if (applyDrawing)
		{
			...
			{
				using var backBrush = new SolidBrush(FormDesign.Design.BackColor.Tint(Lum: FormDesign.Design.IsDarkTheme.If( 6, -6)));
