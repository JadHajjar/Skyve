...
public class DlcListControl : SlickStackedListControl<IDlcInfo, DlcListControl.Rectangles>
{
	...
	protected override void OnPaintItemGrid(ItemPaintEventArgs<IDlcInfo, Rectangles> e)
	{
		...
		DrawIncludedButton(e, isIncluded);

		var height = DrawTitleAndTagsAndVersion(e);

		if (!_dlcManager.IsAvailable(e.Item.Id))
		{
			e.Graphics.DrawLabel(e.Item.IsFree ? Locale.Free : e.Item.Price, null, FormDesign.Design.GreenColor.MergeColor(FormDesign.Design.BackColor), e.Rects.IconRect.Pad(UI.Scale(4)), ContentAlignment.BottomRight);
		}
		else
		{
			e.Graphics.DrawLabel(Locale.Owned, null, FormDesign.Design.ActiveColor.MergeColor(FormDesign.Design.BackColor), e.Rects.IconRect.Pad(UI.Scale(4)), ContentAlignment.BottomRight);
		}
	...
	private void DrawIncludedButton(ItemPaintEventArgs<IDlcInfo, Rectangles> e, bool isIncluded)
	{
		if (!_dlcManager.IsAvailable(e.Item.Id))
		{
			e.Rects.IncludedRect = default;
			return;
		}
		...
		var activeColor = isIncluded ? FormDesign.Design.GreenColor : default;
		Color iconColor;

		if (activeColor == default && inclEnableRect.Contains(CursorLocation))
		{
	...
	protected override IDrawableItemRectangles<IDlcInfo> GenerateRectangles(IDlcInfo item, Rectangle rectangle, IDrawableItemRectangles<IDlcInfo> current)
	{
		...
		rects.TextRect = new Rectangle(rects.IconRect.X, rects.IconRect.Bottom + UI.Scale(4), rects.IconRect.Width - UI.Scale(32), font.Height + UI.Scale(2));

		rects.IncludedRect = new Rectangle(new(rects.IconRect.Right - UI.Scale(28), rects.IconRect.Bottom + UI.Scale(4)), UI.Scale(new Size(28, 28)));
