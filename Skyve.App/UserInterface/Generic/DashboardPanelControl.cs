...
public partial class DashboardPanelControl : SlickControl
{
	private Func<Dictionary<string, DashboardSetting>> _getDefaultLayout;
	...
	public DashboardPanelControl(string? id, Func<Dictionary<string, DashboardSetting>> defaultLayout)
	{
		_id = id;
		_getDefaultLayout = defaultLayout;
		...
		_saveHandler.Load(out _dashItemSizes, $"DashboardLayout{(id is null ? null : $"_{id}")}.json");

		if (_dashItemSizes is null || _dashItemSizes.Count == 0)
		{
			_settings.SessionSettings.DashboardFirstTimeShown = false;
			_dashItemSizes = _getDefaultLayout();
		}
	...
	protected void GlobalMouseMove(object sender, Point p)
	{
	...
	{
		Program.MainForm.OnNextIdle(P_Container.PerformLayout);
	}
	...
	private void LoadItems(IEnumerable<Type> types)
	{
		...
		{
			var id = type.GetCustomAttribute<DashboardItemAttribute>()?.Id;
	...
	{
		_saveHandler.Save(_dashItemSizes, $"DashboardLayout{(_id is null ? null : $"_{_id}")}.json");
		_settings.SessionSettings.DashboardFirstTimeShown = true;
		_settings.SessionSettings.Save();
	}
	...
	private void P_Board_MouseClick(object sender, MouseEventArgs e)
	{
		...
		{
			_saveHandler.Load(out _dashItemSizes, $"DashboardLayout{(_id is null ? null : $"_{_id}")}.json");
